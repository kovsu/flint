...
| **[`stylisticStrict`](#stylisticstrict)** | ☑️ When Ready | Additional rules for consistent styling and best JSX stylistic practices for JSX. |
...
Additional rules that enforce consistent JSX styling and best JSX stylistic practices for most-to-all JavaScript and TypeScript files.
...
export default defineConfig({
	use: [
		{
			files: jsx.files.all,
			rules: jsx.presets.stylisticStrict,
		},
	],
});
```
